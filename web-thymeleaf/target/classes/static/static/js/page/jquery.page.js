...
;(function($){
	$.fn.pager = function(options){
		...
		$.fn.pager.defaults = {showPage:true,"pagesize":[10,20,30],"start":0,"length":10,"total":0,"onselectpage":function(i,len){}};
		...
		function initPage(){
			...
			page.nextPage = function(){
				...
				page.start = tryStart < page.total ? tryStart : page.start;
			};
		...
		{
			var html="";
			if(page.showPage==false || page.showPage=='false'){
				html +=		  '<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+page.currentPage()+'</a>';
				return html;
			}
			...
			var startAndEndNum = 3;//前部分和末尾固定保留个数
			var midNum =2;//中间部分,当前选择的前后保留个数
			if(page.totalPage()> (2*startAndEndNum))
			{/*
			...
			*/
				...
				var firstPoint = 0;//第一个...的位置
				var secendPoint = total - (startAndEndNum+1);//第二个...的位置
				if(curr > (2*startAndEndNum)){
					firstPoint = curr - startAndEndNum;
				}
				if(curr >= total - (2*startAndEndNum-1)){
					secendPoint = 0;
				}
				if(firstPoint == 0 && secendPoint > 0){
					for(var i=1;i <= curr+midNum; i++){
						if(i==page.currentPage())
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+fmtNum(i)+'</a>';
						else
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn">'+fmtNum(i)+'</a>';
					...
					html +=	'<a href="#" class="btn-xs space disabled">...</a>';
					for(var i=total-(startAndEndNum-1);i <= total; i++){
						if(i==page.currentPage())
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+fmtNum(i)+'</a>';
						else
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn">'+fmtNum(i)+'</a>';
					}
				}else if(secendPoint == 0 && firstPoint > 0){
					for(var i=1;i <= startAndEndNum; i++){
						if(i==page.currentPage())
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+fmtNum(i)+'</a>';
						else
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn">'+fmtNum(i)+'</a>';
					...
					html +=	'<a href="#" class="btn-xs space disabled">...</a>';
					for(var i=curr-midNum;i <= total; i++){
						if(i==page.currentPage())
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+fmtNum(i)+'</a>';
						else
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn">'+fmtNum(i)+'</a>';
					}
				}else if(firstPoint == 0 && secendPoint == 0){
					...
					html +=	'<a href="#" class="btn-xs space disabled">...</a>';
					for(var i=curr-midNum;i <= curr+midNum; i++){
						if(i==page.currentPage())
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn pageSelector-current">'+fmtNum(i)+'</a>';
						else
							html +=	'<a href="javascript:void(0)" class="btn-xs space numBtn">'+fmtNum(i)+'</a>';
